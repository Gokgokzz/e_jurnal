...
<body class="bg-[#F4F7FE] text-slate-800 font-sans antialiased">
    ...
    <div class="flex min-h-screen">
        <aside class="w-64 bg-white border-r border-slate-100 flex flex-col justify-between p-6 fixed h-full">
            <div>
                <div class="flex items-center gap-3 mb-10">
                    <div class="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md">E</div>
                    <div>
                ...
                <nav class="space-y-1.5">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-4 px-4 py-3 text-slate-400 hover:text-blue-600 rounded-xl font-medium text-sm transition">
                        <span class="w-5 text-center"><i class="fa-solid fa-chart-pie text-base"></i></span> Dashboard
                    </a>
                    <a href="#" class="flex items-center gap-4 px-4 py-3 bg-blue-50 text-blue-600 rounded-xl font-semibold text-sm transition">
                        <span class="w-5 text-center"><i class="fa-solid fa-pen-to-square text-base"></i></span> Input Jurnal
                    </a>
                    <a href="#" class="flex items-center gap-4 px-4 py-3 text-slate-400 hover:text-slate-600 rounded-xl font-medium text-sm transition">
                        <span class="w-5 text-center"><i class="fa-solid fa-folder-open text-base"></i></span> Rekapitulasi
                    </a>
                </nav>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full flex items-center gap-4 px-4 py-3 text-red-500 hover:bg-red-50 rounded-xl font-semibold text-sm transition text-left">
        ...
        <main class="flex-1 ml-64 p-8">
            ...
            @if(session('success'))
            <div class="bg-emerald-50 border border-emerald-200 text-emerald-600 px-6 py-4 rounded-2xl mb-6 flex items-center gap-3">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-white p-8 rounded-[30px] shadow-sm border border-slate-50">
                            <div class="flex items-center gap-3 mb-6">
                                <div class="w-8 h-8 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center text-xs"><i class="fa-solid fa-info-circle"></i></div>
                                <h4 class="font-bold text-slate-800">Informasi Jurnal</h4>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Tanggal Mengajar</label>
                                    <input type="date" name="tanggal" value="{{ date('Y-m-d') }}" class="w-full bg-[#F4F7FE] border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition">
                                </div>
                                <div>
                                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Pilih Kelas</label>
                                    <select name="kelas_id" class="w-full bg-[#F4F7FE] border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition">
                                ...
                                <div>
                                    ...
                                    <select name="mapel_id" class="w-full bg-[#F4F7FE] border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition">
                                ...
                                <div>
                                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Jam Pelajaran</label>
                                    <input type="text" name="jam_pelajaran" placeholder="Contoh: 1 - 4" required class="w-full bg-[#F4F7FE] border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition">
                                </div>
                            </div>

                            <div class="mt-6">
                                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">Materi Pembelajaran (Ringkasan)</label>
                                <textarea name="materi" rows="4" placeholder="Tuliskan pokok bahasan hari ini..." class="w-full bg-[#F4F7FE] border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 transition"></textarea>
                            </div>
                        </div>
                        ...
                        <div class="bg-white p-8 rounded-[30px] shadow-sm border border-slate-50">
                            <div class="flex justify-between items-center mb-6">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 bg-orange-50 text-orange-600 rounded-lg flex items-center justify-center text-xs"><i class="fa-solid fa-users-check"></i></div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left text-sm border-collapse">
                                    ...
                                    <tbody id="tabel-siswa" class="divide-y divide-slate-50">
                                        <tr>
                                            <td colspan="6" class="py-6 text-center text-xs text-slate-400 font-medium">Silakan pilih kelas terlebih dahulu.</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white p-8 rounded-[30px] shadow-sm border border-slate-50 sticky top-8">
                            ...
                            <p class="text-[11px] text-slate-400 font-medium leading-relaxed mb-6">Pastikan data materi dan absensi siswa sudah sesuai sebelum menekan tombol simpan.</p>
                            
                            <div class="space-y-3">
                                <button type="submit" class="w-full bg-[#5680F9] hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-blue-100 transition flex items-center justify-center gap-3 text-sm">
                                    <i class="fa-solid fa-cloud-arrow-up"></i> Simpan Jurnal
                                </button>
                                <a href="{{ route('dashboard') }}" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-4 rounded-2xl transition flex items-center justify-center text-sm">
                                    Batalkan
                                </a>
                            </div>
                            ...
                            <div class="flex items-center gap-4 text-slate-400">
                                <div class="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-lg"><i class="fa-solid fa-shield-halved"></i></div>
                                <div>
                                    <p class="text-[10px] font-bold uppercase tracking-wider">Validasi Sistem</p>
                                    <p class="text-[10px] font-medium leading-tight">Data akan langsung masuk ke rekapitulasi sekolah.</p>
                                </div>
                            </div>
                        </div>
                    </div>
